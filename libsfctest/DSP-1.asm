...
;.IF (MAP_MODE&1) == 1
;.define _DSP_IO			$6000 ;HIROM
;.define _DSP_DATA_REG	$6000
;.define _DSP_STATUS_REG	$7000 ;Status register
;.ELSE
.define _DSP_IO					$8000 ;LOROM
.define _DSP_IO_LONG			$8000 ;LOROM
.define _DSP_DATA_REG			$8000
.define _DSP_DATA_REG_LONG		$3F8000
.define _DSP_STATUS_REG			$C000 ;LOROM
.define _DSP_STATUS_REG_LONG	$3FC000
.define _DSP_BANK				$3F
;.ENDIF

.macro WaitRQM
_rqm\@:	BIT.w _DSP_STATUS_REG ; Only works when in banks $30-3F
...
.ramsection "libsfc_video_variables" bank $7F slot 3
_WRAM_HDMAEN dsb 1
.ends


.define _OFFSET_CX -128
.define _OFFSET_CY -112

.define _hdma_matrix1_size ((127<<2))
.define _hdma_matrix2_size ((97<<2)+1)

.base $00
.ramsection "dsp1_perspective_proj_variables" bank $7E slot 2
;TODO: rewrite functions to use C DSP_Perspective and remove these
_cam_x dw	; [CI] horizontal
_cam_y dw	; [CI] depth
_cam_z dw	; [CI] vertical
_cam_Lfe dw	; [U] Distance between base point and viewpoint (Sets screen-sprite ratio)
_cam_Les dw	; [U] fov, Distance between viewpoint and screen (the effect of screen angle considered; screen horizontal distance 256)
_cam_Aas dw	; [A] Azimuth angle, rotation around Y axis
_cam_Azs dw	; [A] Zenith angle, X/Z rotationish
_cam_cx dw	;\ [CI] focal point
_cam_cy dw	;/ 
_rastercenter dw ; [I]
_horizon dw ; [I]

;TODO: write hdma routines
_hdmaMatrixPointerAB dsb (1+2 + 1+2 + 1+2);1 byte repeat, 2 byte pointer for each entry
_hdmaMatrixPointerCD dsb (1+2 + 1+2 + 1+2)

_hdmaMatrixAB1 dsb _hdma_matrix1_size
_hdmaMatrixAB2 dsb _hdma_matrix2_size
_hdmaMatrixCD1 dsb _hdma_matrix1_size
_hdmaMatrixCD2 dsb _hdma_matrix2_size

.ends

.enum $0000
	__c_DSP_perspective_struct: .db
	__c_DSP_perspective_struct_x dw
	__c_DSP_perspective_struct_y dw
	__c_DSP_perspective_struct_z dw
	__c_DSP_perspective_struct_Lfe dw
	__c_DSP_perspective_struct_Les dw
	__c_DSP_perspective_struct_Aas dw
	__c_DSP_perspective_struct_Azs dw
	__c_DSP_perspective_struct_center_x dw
	__c_DSP_perspective_struct_center_y dw
	__c_DSP_perspective_struct_raster_center dw
	__c_DSP_perspective_struct_horizon dw
.ende

.base $80
.section "libsfc_dsp-1_projection_assembly_code" superfree
...
.section "other_dsp1_stuff" superfree
