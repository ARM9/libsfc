#ifndef _LIBSFC_REGISTERS
#define _LIBSFC_REGISTERS
#include <stdint.h>

//All hw registers should be handled as volatile so vu8_reg is superfluous
//not that it matters since tcc doesn't optimize anything but whatever...
#define u8_reg(addr) (*(vu8*)addr)
#define u16_reg(addr) (*(vu16*)addr) //Convenient for $43xx register access

#define u8_addr(a) (*(u8*)a)
#define s8_addr(a) (*(s8*)a)
#define u16_addr(a) (*(u16*)a)
#define s16_addr(a) (*(s16*)a)
...
#define REG_M7D			(*(vu8*)0x211E) // | format is 16 bit two's complement signed, 7.8 fixeds if you will
...
#define REG_MPYLM		(*(vu16*)0x2134)// Mode7 A*B product, pointer to low 16 bits
#define REG_MPYMH		(*(vu16*)0x2135)// product, pointer to high 16 bits
...
#define REG_RDDIVL	(*(vu8*)0x4214) // Quotient of divide low byte
#define REG_RDDIVH	(*(vu8*)0x4215) // high byte
